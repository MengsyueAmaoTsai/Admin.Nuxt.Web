  ...
  <div>
    ...
    <table :id="props.id" :class="gridClass()" :style="styleValue">
      <thead v-if="props.generateHeader !== GenerateHeaderOption.None">
        <BaseDataGridRow>
          <BaseDataGridCell v-for="(column, columnIndex) in columns">
          </BaseDataGridCell>
        </BaseDataGridRow>
      </thead>
      ...
      <tbody>
        ...
        <BaseDataGridRow
          ...
          :item="item"
          :class="props.rowClass ? props.rowClass(item) : undefined"
          :style="props.rowStyle ? props.rowStyle(item) : undefined"
        >
          <BaseDataGridCell
...
const props = defineProps({
  ...
  resizeType: {
    type: String as PropType<DataGridResizeType>,
  },
  ...
  noTabbing: Boolean,
  generateHeader: {
    type: Function as PropType<GenerateHeaderOption>,
...
const columns = [{}, {}];
const collectingColumns = true;

const manualGrid = false;
</script>
