...
export function initialize(gridElement: HTMLElement, autoFocus: boolean) {
	...
	let start = gridElement.querySelector("td:first-child") as HTMLElement;
...
export function enableColumnResizing(gridElement: HTMLElement) {
	const columns = [];
	let min = 75;
	...
	headers.forEach((header) => {
			...
			requestAnimationFrame(() => {
				...
				if (document.body.dir === "" || document.body.dir === "ltr") {
					...
					width =
						...
						headerBeingResized.clientWidth -
						(horizontalScrollOffset + e.clientX);
				}
		...
		header
			...
			.addEventListener("pointerdown", initResize);
	});
	...
	if (gridElement.style.gridTemplateColumns) {
		...
		initialWidths = columns.map(({ header, size }) => size).join(" ");
...
export function resizeColumnDiscrete(gridElement, column, change) {
	...
	if (!column) {
		...
		if (
			!(
				targetElement.classList.contains("column-header") &&
				targetElement.classList.contains("resizable")
			)
		) {
			return;
		}
		...
		headerBeingResized = gridElement.querySelector(
			'.column-header[col-index="' + column + '"]',
		);
	}

	// biome-ignore lint/complexity/noForEach: <explanation>
	grids
		.find(({ id }) => id === gridElement.id)
		.columns.forEach((column) => {
...
export function resizeColumnExact(gridElement, column, width) {
	const columns = [];
	let headerBeingResized = gridElement.querySelector(
		'.column-header[col-index="' + column + '"]',
	);
...
export function autoFitGridColumns(gridElement, columnCount) {
	...
	for (var i = 0; i < columnCount; i++) {
		const columnWidths = Array.from(
			gridElement.querySelectorAll(`[col-index="${i + 1}"]`),
